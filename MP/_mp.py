...
class MpModel:
    def __init__(self, PATH_DATA, dataset="AML") -> None:
        if dataset == "AML":
        ...
        if dataset == "PD":
            self.Y = np.array([self.ct2idx[_] for _ in adata.obs.cell_type])
        else:
            self.Y = np.array([self.ct2idx[_] for _ in df.cell_type])
